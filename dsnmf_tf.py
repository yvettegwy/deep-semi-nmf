...
def appr_seminmf(M, r):
    ...
    AA = A[:, ii]
    BB = B[ii, :]
    Atemp = tf.cond(tf.less(tf.reduce_min(B[ii, :]), tf.reduce_min(tf.negative(B[ii, :]))), lambda: tf.reshape(tf.negative(AA),[m,1]), lambda: tf.reshape(AA,[m,1]))
    Btemp = tf.cond(tf.less(tf.reduce_min(B[ii, :]), tf.reduce_min(tf.negative(B[ii, :]))), lambda: tf.reshape(tf.negative(BB),[1,n]), lambda: tf.reshape(BB,[1,n]))
    if r > 2:
        for i in range(1,r-1):
            ii = tf.constant(i)
            AA = tf.reshape(A[:, ii], [m,1])
            BB = tf.reshape(B[ii, :], [1,n])
            Atemp = tf.cond(tf.less(tf.reduce_min(B[ii, :]), tf.reduce_min(tf.negative(B[ii, :]))), lambda: tf.concat([Atemp, tf.negative(AA)], axis = 1), lambda: tf.concat([Atemp, AA], axis = 1))
            Btemp = tf.cond(tf.less(tf.reduce_min(B[ii, :]), tf.reduce_min(tf.negative(B[ii, :]))), lambda: tf.concat([Btemp, tf.negative(BB)], axis = 0), lambda: tf.concat([Btemp, BB], axis = 0))
